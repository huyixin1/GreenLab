...
def measure_knn(r,f):
    ...
    with pyRAPL.Measurement(label, output=csv_output):
        # load data
        x_train_data = pd.read_csv('data/processed/' + str(r) + 'r_' + str(f) + 'f' + '_train_data.csv')
        y_train_data = pd.read_csv('data/processed/' + str(r) + 'r_' + str(f) + 'f' + '_train_labels.csv')
        x_val_data = pd.read_csv('data/processed/' + str(r) + 'r_' + str(f) + 'f' + '_val_data.csv')
        y_val_labels = pd.read_csv('data/processed/' + str(r) + 'r_' + str(f) + 'f' + '_val_labels.csv')


        # make and fit kNN model
        NeighbourModel = KNeighborsClassifier(n_neighbors=5)
        NeighbourModel.fit(x_train_data, y_train_data.values.ravel())

        # make report and add to dataframe
        report = classification_report(y_val_labels, NeighbourModel.predict(x_val_data), output_dict=True)

        # save data to csv file
        df = pd.DataFrame([report['accuracy']])
        df.columns = ['Accuracy']
        df.to_csv('Accuracies/knn_' + str(r) + 'r_' + str(f) + 'f' + '.csv', index=False)
...
def measure_random_forest(r,f):
    ...
    with pyRAPL.Measurement(label, output=csv_output):
        ...
        # make and fit Random Forest model
        RandomForestModel = RandomForestClassifier(n_estimators=100, random_state=25)
        RandomForestModel.fit(x_train_data, y_train_data.values.ravel())

        # make report and add to dataframe
        report = classification_report(y_val_labels, RandomForestModel.predict(x_val_data), output_dict=True)

        # save data to csv file
        df = pd.DataFrame([report['accuracy']])
        df.columns = ['Accuracy']
        df.to_csv('Accuracies/random_forest_adult_' + str(r) + 'r_' + str(f) + 'f' + '.csv', index=False)
